...
public class AnchorPaneApp extends Application{
	...
	//앱이 가동될 때(Application Thread: UI, Event 전용 쓰레드에 의해 호출)
	//무조건 호출됨, 따라서 앱이 가동될 때 주로 디자인 구성을 여기서 처리하면 된다.
	public void start(Stage stage) throws Exception {
		//stage? 이 앱에서 사용될 윈도우를 stage라 한다.
		
		//윈도우 안에는 반드시 Scene이 와야 한다.
		//Scene은 컨테이너 즉, 레이아웃 객체이다.
		//하나의 윈도우 안에는 하나의 Scene이 올 수 있다.
		AnchorPane parent = new AnchorPane();//absolute(절대 위치)로 위피를 지정할 수 있는 pane
